...
AttitudeController::AttitudeController()
  : DELTA_T(0), prev_time_(-1), use_ground_truth_(false), sim_voltage_(0), gimbal_dof_(0)
{
}
...
void AttitudeController::init(ros::NodeHandle* nh, StateEstimate* estimator)
{
  ...
  torque_allocation_matrix_inv_sub_ =
      nh_->subscribe("torque_allocation_matrix_inv", 1, &AttitudeController::torqueAllocationMatrixInvCallback, this);
...
void AttitudeController::update(void)
{
  ...
  if (start_control_flag_ && att_control_flag_)
  {
...
#ifdef SIMULATION
      ROS_ERROR("failsafe: cannot connect with ROS. time now: %d, time last stamp: %d", HAL_GetTick(),
                flight_command_last_stamp_);
#else
    ...
    ROS_DEBUG_THROTTLE(0.01,
                       "true vs spinal: r [%f vs %f], p [%f vs %f], y [%f vs %f], ws [%f vs %f], wy [%f vs %f], wz [%f "
                       "vs %f]",
                       true_angles_.x, angles.x, true_angles_.y, angles.y, true_angles_.z, angles.z, true_ang_vel_.x,
                       ang_vel.x, true_ang_vel_.y, ang_vel.y, true_ang_vel_.z, ang_vel.z);

    if (prev_time_ < 0)
      DELTA_T = 0;
    else
      DELTA_T = ros::Time::now().toSec() - prev_time_;
    ...
    // linear control method
    {
      ...
      for (int i = 0; i < motor_number_; i++)
      {
        ...
        float gyro_moment_compensate = p_matrix_pseudo_inverse_[i][0] * gyro_moment.x +
...
void AttitudeController::fourAxisCommandCallback(const spinal::FourAxisCommand& cmd_msg)
{
  ...
  {
    ROS_ERROR("fource axis commnd: motor number is not identical between fc(%d) and pc(%ld)", motor_number_,
              cmd_msg.base_thrust.size());
    return;
  }
#else
...
void AttitudeController::rpyGainCallback(const spinal::RollPitchYawTerms& gain_msg)
{
  ...
  {
    ROS_ERROR("rpy gain: motor number is not identical between fc:%d and pc:%d", motor_number_,
              (int)gain_msg.motors.size());
    return;
  }
#else
...
{
  if (motor_number_ == 0 || !start_control_flag_)
    return;

#ifdef SIMULATION
  if (msg.rows.size() != motor_number_)
  {
    ROS_ERROR("torqueAllocationMatrixInvCallback: motor number is not identical between fc(%d) and pc(%ld)",
              motor_number_, msg.rows.size());
    return;
  }
#else
...
void AttitudeController::pMatrixInertiaCallback(const spinal::PMatrixPseudoInverseWithInertia& msg)
{
  ...
  {
    if (motor_number_ > 0)
      ROS_ERROR("p matrix pseudo inverse and inertia commnd: motor number is not identical between fc and pc");
    return;
  }
#else
...
bool AttitudeController::activated()
{
  ...
  if (motor_number_ > 0 && uav_model_ >= spinal::UavInfo::DRONE && max_duty_ > min_duty_)
    return true;
  else
    return false;
}
...
void AttitudeController::pwmConversion()
{
    ...
    switch (pwm_conversion_mode_)
    {
      case spinal::MotorInfo::SQRT_MODE: {
        ...
        float sqrt_tmp =
            ...
            4 * 10 * motor_info_[motor_ref_index_].polynominal[2] *
                (motor_info_[motor_ref_index_].polynominal[0] - scaled_thrust);  // special decimal order shift (x10)
  ...
  if (HAL_GetTick() - voltage_update_last_time_ > 500)  //[500ms = 0.5s]
  {
    ...
    switch (pwm_conversion_mode_)
    {
      case spinal::MotorInfo::SQRT_MODE: {
        /* pwm = F_inv[(V_ref / V)^2 f] */
        v_factor_ =
            (motor_info_[motor_ref_index_].voltage / voltage) * (motor_info_[motor_ref_index_].voltage / voltage);
        break;
      }
      case spinal::MotorInfo::POLYNOMINAL_MODE: {
        /* pwm = F_inv[(V_ref / V)^1.5 f] */
  ...
  for (int i = 0; i < motor_number_ / (gimbal_dof_ + 1); i++)
  {
    ...
    if (gimbal_dof_)
    {
      thrust = ap::pythagorous2(base_thrust_term_[2 * i] + roll_pitch_term_[2 * i],
                                base_thrust_term_[2 * i + 1] + roll_pitch_term_[2 * i + 1]);
    }
    else
    {
    ...
    else
    {
      if (max_yaw_term_index_ != -1 && fabs(base_thrust_term_[0]) > 0)
      {
        ...
        for (int i = 0; i < motor_number_ / (gimbal_dof_ + 1); i++)
        {
          ...
          {
            thrust =
                ap::pythagorous2(base_thrust_term_[2 * i] + roll_pitch_term_[2 * i],
                                 base_thrust_term_[2 * i + 1] + roll_pitch_term_[2 * i + 1] + yaw_term_[2 * i + 1]);
          }
          else
          {
            thrust = base_thrust_term_[i] + roll_pitch_term_[i];
        ...
        if (yaw_decreasing_rate < -1)
          yaw_decreasing_rate = -1;
        if (yaw_decreasing_rate > 0)
          yaw_decreasing_rate = 0;
      }
      else
      {
        yaw_decreasing_rate = -1;
  ...
  for (int i = 0; i < motor_number_; i++)
    target_thrust_[i] = roll_pitch_term_[i] + (1 + base_thrust_decreasing_rate) * base_thrust_term_[i] +
                        (1 + yaw_decreasing_rate) * yaw_term_[i];
