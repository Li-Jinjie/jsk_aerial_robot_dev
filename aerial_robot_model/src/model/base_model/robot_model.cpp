...
RobotModel::RobotModel(bool init_with_rosparam, bool verbose, bool fixed_model, double fc_f_min_thre,
...
KDL::RigidBodyInertia RobotModel::inertialSetup(const KDL::TreeElement& tree_element)
{
  ...
  if (verbose_)
    ROS_WARN_STREAM("segment " << current_seg.getName() << ", mass is: " << current_seg_inertia.getMass());
  ...
  if (current_seg.getName().find("root") != std::string::npos)
  {
    ...
    if (verbose_)
      ROS_WARN("Add root link: %s", child_seg.getName().c_str());
  }
  ...
  if (current_seg.getJoint().getType() != KDL::Joint::None)
  {
    ...
    if (current_seg.getJoint().getName().find("rotor") == std::string::npos)
    {
      ...
      if (verbose_)
        ROS_WARN("Add new inertia base link: %s", current_seg.getName().c_str());
    }
  }
  ...
  if (current_seg.getJoint().getName().find("rotor") != std::string::npos)
  {
    ...
    {
      if (verbose_)
        ROS_WARN("joint name: %s, z axis: %f", current_seg.getJoint().getName().c_str(), urdf_joint->axis.z);
  ...
  for (const auto& elem : GetTreeElementChildren(tree_element))
  {
    ...
    if (verbose_)
      ROS_WARN("Add new child segment %s to direct segment: %s", child_seg.getName().c_str(),
               current_seg.getName().c_str());
  }

  /* count the rotor */
  if (current_seg.getName().find(thrust_link_.c_str()) != std::string::npos)
    rotor_num_++;
  ...
  if (inertia_map_.find(current_seg.getName()) != inertia_map_.end())
  {
    ...
    if (verbose_)
      ROS_WARN("Total mass of base segment %s is %f", current_seg.getName().c_str(),
                                ...
                                KDL::RigidBodyInertia inertia)
{
  if (extra_module_map_.find(module_name) == extra_module_map_.end())
  {
    ...
    {
      ROS_WARN("[extra module]: fail to add new extra module %s, because its parent link (%s) does not exist",
    ...
    {
      ROS_WARN("[extra module]: fail to add new extra module %s, because its orientation is invalid",
               module_name.c_str());
      return false;
    }
    ...
    {
      ROS_WARN("[extra module]: fail to add new extra module %s, becuase its mass %f is invalid", module_name.c_str(),
               inertia.getMass());
      return false;
    }
...
void RobotModel::updateRobotModelImpl(const KDL::JntArray& joint_positions)
{
  ...
  for (int i = 0; i < rotor_num_; ++i)
  {
    ...
    if (verbose_)
      ROS_WARN(" %s : [%f, %f, %f]", rotor.c_str(), f.p.x(), f.p.y(), f.p.z());
...
KDL::Frame RobotModel::forwardKinematicsImpl(std::string link, const KDL::JntArray& joint_positions) const
{
  ...
  if (status < 0)
    ROS_ERROR("can not solve FK to link: %s", link.c_str());
...
bool RobotModel::stabilityCheck(bool verbose)
{
  ...
  {
    if (verbose)
      ROS_ERROR_STREAM("the min distance to the plane of feasible control force convex "
  ...
  {
    if (verbose)
      ROS_ERROR_STREAM("the min distance to the plane of feasible control torque convex "
...
TiXmlDocument RobotModel::getRobotModelXml(const std::string param, ros::NodeHandle nh)
{
  ...
  {
    ROS_ERROR("Could not find parameter %s on parameter server with namespace '%s'", param.c_str(),
