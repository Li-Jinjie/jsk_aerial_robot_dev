...
class NMPCTiltBiServo(QDNMPCBase):
    """
    NMPC controller for tiltable birotor (2 rotors).
    ...
    def __init__(self, build: bool = True):
        ...
        # Set number of rotors for birotor configuration
        self.num_rotors = 2
    ...
    def get_cost_function(self, **kwargs):
        """
        Define the cost function for the birotor NMPC.
    ...
    def get_weights(self):
        # Define weights for birotor (2 rotors, 2 servo angles)
        # State weights: p(3), v(3), qw_ref(1), qe(3), w(3), a(2)
        Q = np.diag(
            [
        ...
        # Control weights: ft(2), a_c_derivative(2)
        R = np.diag(
            [
                self.params["Rt"],  # ft1
                self.params["Rt"],  # ft2
    ...
    def _create_reference_generator(self) -> BINMPCReferenceGenerator:
        """
        Create the reference generator for birotor.
        Overrides the base class method to use BINMPCReferenceGenerator instead of QDNMPCReferenceGenerator.
        """
        # fmt: off
