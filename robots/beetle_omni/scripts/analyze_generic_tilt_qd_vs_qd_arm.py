...
def plot_force_response(data: dict[str, np.ndarray], output_path: Path) -> None:
    ...
    axes[0].plot(force, data["qd_pitch_deg"], label="qd+arm pitch", linewidth=2.0)
    axes[0].plot(force, data["tilt_pitch_deg"], label="tilt-qd pitch", linewidth=2.0)
    axes[0].set_ylabel("Attitude [deg]")
    axes[0].legend(framealpha=0.9)

    axes[1].plot(force, data["qd_tau_y_abs"], label=r"qd+arm $|\tau_y|$", linewidth=2.0)
    axes[1].plot(force, data["tilt_servo_tau"], label="tilt-qd servo torque", linewidth=2.0)
    ...
    axes[2].legend(ncol=2, fontsize=9, framealpha=0.9)
...
def plot_coupled_wrench_envelope(
    ...
    ax.plot(force_values, qd_tau_pos, color="C0", linewidth=2.0, label=r"qd+arm $+\tau_x$")
    ax.plot(force_values, qd_tau_neg, color="C0", linewidth=2.0, linestyle="--", label=r"qd+arm $-\tau_x$")
    ax.plot(force_values, tilt_tau_pos, color="C1", linewidth=2.0, label=r"tilt-qd $+\tau_x$")
    ax.plot(force_values, tilt_tau_neg, color="C1", linewidth=2.0, linestyle="--", label=r"tilt-qd $-\tau_x$")
    ax.fill_between(force_values, qd_tau_neg, qd_tau_pos, color="C0", alpha=0.10)
    ax.fill_between(force_values, tilt_tau_neg, tilt_tau_pos, color="C1", alpha=0.10)
...
def main() -> None:
    ...
    parser.add_argument("--response-plot", type=Path, default=Path("generic_tilt_qd_vs_qd_arm_response.png"))
    parser.add_argument("--envelope-plot", type=Path, default=Path("generic_tilt_qd_vs_qd_arm_envelope.png"))
    ...
    plot_force_response(data, args.response_plot)
